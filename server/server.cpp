...
//    char *local_addr="127.0.0.1";
 ...
 //   inet_aton(local_addr,&(serveraddr.sin_addr));//htons(portnumber);
    ...
    rcd = bind(listenfd,(sockaddr *)&serveraddr, sizeof(serveraddr));
		if (rcd == -1) {
			printf("bind port %d error!\n", ntohs(serveraddr.sin_port));
			exit(1);
		}    
    
    rcd = listen(listenfd, LISTENQ);
		if (rcd == -1) {
			printf("listen error!\n");
			exit(1);
		}    
...
//                char *str = inet_ntoa(clientaddr.sin_addr);
                cout << "accapt a connection from " << inet_ntoa(clientaddr.sin_addr) << " on socket " << connfd << endl;
                //设置用于读操作的文件描述符
